...
# -----------------------------------------------------------------------------
# 4. HİPERPARAMETRE OPTİMİZASYONU (GÜNCELLENMİŞ - REGULARIZATION EKLENDİ)
# -----------------------------------------------------------------------------
print("\n⚙️ Hiperparametre Optimizasyonu: Overfitting Önleyici Ayarlar...")

# Parametre Uzayı (Fren mekanizmaları eklendi)
param_dist = {
    # 1. Ağaç Yapısı
    'n_estimators': [500, 1000],
    'learning_rate': [0.03, 0.05],
    'max_depth': [3, 4, 5],  # 4'ü özellikle ekledik (Ara form)

    # 2. Rastsallık (Ezberi bozar)
    'subsample': [0.7, 0.8],
    'colsample_bytree': [0.7, 0.8],

    # 3. REGULARIZATION (YENİ - FREN SİSTEMİ)
    # Modelin ağırlıklarını büyütmesini engeller (Overfitting düşmanı)
    'reg_alpha': [0, 0.5, 1],  # L1 Regularization (Gereksiz özellikleri sıfırlar)
    'reg_lambda': [1, 5, 10],  # L2 Regularization (Aşırı uyumu törpüler)
...
random_search = RandomizedSearchCV(
    ...
    n_iter=30,  # 30 deneme ile arayıp bulsun
...
# -----------------------------------------------------------------------------
# 5. FİNAL MODELİN EĞİTİLMESİ (EARLY STOPPING DÜZELTİLMİŞ HALİ)
# -----------------------------------------------------------------------------
print("\n🦾 Final Model Eğitiliyor (Akıllı Durdurma Aktif)...")

# En iyi parametreli modeli alıyoruz
best_model = random_search.best_estimator_

# DÜZELTME BURADA: Parametreyi modelin kendisine ekliyoruz
# Eğer XGBoost sürümün çok yeniyse bu yöntem, eskiyse callbacks yöntemi çalışır.
# En garanti yöntem "set_params" kullanmaktır.
best_model.set_params(early_stopping_rounds=50)

eval_set = [(X_train, y_train), (X_test, y_test)]

# fit() içinden 'early_stopping_rounds' parametresini SİLİYORUZ.
# Model zaten yukarıdaki ayar sayesinde bunu biliyor olacak.
best_model.fit(
    X_train, y_train,
    eval_set=eval_set,
    verbose=False
